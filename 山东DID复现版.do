/*===========================================================
  山东数字乡村DID实证分析 - 复现版
  使用说明：
    1. 将本 .do 文件与「山东DID中文.xlsx」放在同一文件夹
    2. 在 Stata 中运行：do "山东DID复现版.do"
    3. 所有输出结果将保存在同一文件夹内
  依赖包：estout、coefplot（首次运行自动安装）
===========================================================*/
...
* ── 导入数据（相对路径）──────────────────────────────────
import excel "山东DID中文.xlsx", sheet("完整面板数据") cellrange(A4:AJ3200) clear
...
/*===========================================================
  二、主效应检验（H1）
  digital_lit = β·did + 控制变量 + 个体FE + 时间FE + ε
===========================================================*/
...
esttab m1 m2 m3 using "主效应DID.rtf", ///
...
/*===========================================================
  三、平行趋势检验（事件研究法）
  前1期为基准组；lead3 lead2 [基准] event0 lag1 lag2 lag3plus
===========================================================*/

eststo es: xtreg digital_lit lead3 lead2 event0 lag1 lag2 lag3plus ///
    $controls, fe robust

esttab es using "平行趋势检验.rtf", ///
    b(3) se(3) star(* 0.1 ** 0.05 *** 0.01) ///
...
esttab mech1_s1 mech1_s2 mech2_s1 mech2_s2 using "机制检验.rtf", ///
    b(3) se(3) star(* 0.1 ** 0.05 *** 0.01) ///
...
* 交互项回归（did_hc = did × high_hc，数据中已有）
eststo het_inter: xtreg digital_lit did high_hc did_hc $controls, fe robust

esttab het_low het_high het_inter using "异质性检验.rtf", ///
    b(3) se(3) star(* 0.1 ** 0.05 *** 0.01) ///
...
/*===========================================================
  六、稳健性检验
  6.1 安慰剂检验（500次随机置换）
  6.2 排除济南主城区（county_id 370102-370115）
  6.3 对数被解释变量（见主效应m3）
===========================================================*/
...
esttab robust1 robust2 using "稳健性检验.rtf", ///
    b(3) se(3) star(* 0.1 ** 0.05 *** 0.01) ///
    keep(did) ///
    title("表6 稳健性检验") ///
...
/*===========================================================
  完成
===========================================================*/
di as result "所有分析完成！输出文件保存在：`c(pwd)'"
di as result "  描述性统计.rtf  主效应DID.rtf  平行趋势检验.rtf"
di as result "  机制检验.rtf    异质性检验.rtf  稳健性检验.rtf"
di as result "  平行趋势图.png  安慰剂检验.png"
